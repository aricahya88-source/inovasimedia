...
const empty:D={week_id:'W01',title:'',prompt_html:'',max_score:100,min_posts:1,due_at:'',visible:true};
...
export default function AdminDiscussions(){
  ...
  return <AuthGate adminOnly><AppShell title="Kelola Diskusi">
    <div className="admin-split">
      ...
      <GlassCard>
        <div className="form-grid two"><label className="field"><span>Minggu</span><select value={d.week_id} onChange={e=>setD({...d,week_id:e.target.value})}>{Array.from({length:14},(_,i)=><option key={i} value={`W${String(i+1).padStart(2,'0')}`}>Minggu {i+1}</option>)}</select></label><label className="field"><span>Judul</span><input value={d.title} onChange={e=>setD({...d,title:e.target.value})}/></label><label className="field"><span>Skala penilaian</span><input type="number" value={100} disabled/><small>Diskusi selalu dinilai pada skala 0–100.</small></label><label className="field"><span>Minimal post</span><input type="number" value={d.min_posts} onChange={e=>setD({...d,min_posts:Number(e.target.value)})}/></label></div>
