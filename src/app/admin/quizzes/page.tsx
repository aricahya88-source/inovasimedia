    ...
    <div className="admin-split">
      ...
      <div className="stack">
        ...
        {q.activity_id&&<GlassCard>
          ...
          <div className="quiz-question-list">{q.questions.map((x,i)=><button key={x.question_id} className={question.question_id===x.question_id?'mini-question active':'mini-question'} onClick={()=>setQuestion(x)}><strong>{i+1}</strong><div className="grow" dangerouslySetInnerHTML={{__html:x.question_html}}/><span>bobot {x.points}</span></button>)}</div>
        </GlassCard>}

        {q.activity_id&&<GlassCard>
          <span className="eyebrow">EDITOR SOAL • NILAI AKHIR 0–100</span><div className="form-grid two"><label className="field"><span>Urutan</span><input type="number" value={question.order_no} onChange={e=>setQuestion({...question,order_no:Number(e.target.value)})}/></label><label className="field"><span>Bobot relatif soal</span><input type="number" min="0.1" step="0.1" value={question.points} onChange={e=>setQuestion({...question,points:Number(e.target.value)})}/></label></div>
