...
export default function AdminProjects(){
  ...
  return <AuthGate adminOnly><AppShell title="Kelola Proyek"><div className="admin-split">
    <GlassCard className="admin-list"><span className="eyebrow">PERENCANAAN</span><h3>{rows.length} Proposal</h3><div className="scroll-list">{rows.map(x=><button key={x.plan_id} className={p?.plan_id===x.plan_id?'select-row active':'select-row'} onClick={()=>open(x)}><strong>{x.title||'(Belum berjudul)'}</strong><small>{x.project_code} • {x.group_name?`${x.group_name} • ketua ${x.owner_name||'-'}`:(x.owner_name||'-')} • {x.status}</small></button>)}</div></GlassCard>
    <div className="stack">{p&&<>
      <GlassCard><span className="eyebrow">{p.project_code} • {p.status}</span><h2>{p.title}</h2><p><strong>Pemilik:</strong> {p.group_name?`${p.group_name} — ketua ${p.owner_name||'-'}`:(p.owner_name||'-')} {p.owner_nim?`(${p.owner_nim})`:''}</p><p><strong>Tema:</strong> {p.theme_code} • <strong>Topik:</strong> {p.topic}</p></GlassCard>
      {[
