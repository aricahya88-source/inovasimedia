...
import { ArrowLeft, MessagesSquare, CircleHelp, ClipboardCheck, FolderKanban, ArrowRight } from 'lucide-react';
...
export default function WeekPage({params}:{params:Promise<{week:string}>}){
  ...
  const [d,setD]=useState<WeekData|null>(null); const [error,setError]=useState('');
  useEffect(()=>{api<WeekData>('getWeek',{week_no:Number(week)}).then(setD).catch(e=>setError(e.message));},[week]);
  ...
  return <AuthGate><AppShell title={`Minggu ${week}`}>
    ...
    {!d?<div className="screen-center small"><div className="spinner"/>Memuat materi...</div>:<div className="stack">
      <section className="hero-panel glass-panel"><div><span className="eyebrow">MINGGU {d.week.week_no}</span><h2>{d.week.title}</h2><p>Dua materi asli digabung dalam satu minggu LMS tanpa mengubah susunan kontennya.</p></div></section>
      {d.materials.map(m=><MaterialView key={m.material_id} material={m}/>)}
      <div className="section-title"><div><span className="eyebrow">AKTIVITAS</span><h3>Kuis, diskusi, dan tugas</h3></div></div>
