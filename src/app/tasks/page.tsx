...
import { ClipboardCheck, CircleHelp, FolderKanban, ArrowRight, Zap, CalendarClock } from 'lucide-react';

type StaticRow={activity_id:string;week_id:string;type:string;title:string;max_score?:number;project_code?:string;quiz_id?:string;material_range?:string;due_at?:string};
...
export default function TasksPage(){
  ...
  return <AuthGate><AppShell title="Tugas & Kuis"><div className="section-title"><div><span className="eyebrow">7 KUIS • 5 PROYEK</span><h2>Kuis & Proyek</h2><p className="muted">Kuis hanya tujuh checkpoint. Soal berasal dari static bundle; hanya attempt, nilai, dan data proyek yang tersimpan di Sheets.</p></div><span className="badge success"><Zap/> CDN + Sheets</span></div>{error&&<div className="error-box">{error}</div>}<div className="stack small-gap">{rows.map(r=><Link href={href(r)} key={r.activity_id} className="glass-card list-card"><div className={`icon-bubble ${r.type==='quiz'?'amber':r.type==='project'?'coral':'teal'}`}>{r.type==='quiz'?<CircleHelp/>:r.type==='project'?<FolderKanban/>:<ClipboardCheck/>}</div><div className="grow"><span className="eyebrow">{r.week_id} • {r.type==='quiz'?`MATERI ${r.material_range}`:r.type.toUpperCase()}</span><h3>{r.title}</h3><div className="row wrap gap"><small>Maks. {r.max_score||0} poin</small>{r.type==='quiz'&&<span className="badge">{progress.quizAttempts[r.quiz_id||'']||0} attempt</span>}{r.type==='quiz'&&r.due_at&&<span className="badge"><CalendarClock/> batas blok</span>}{r.type==='project'&&<span className="badge success">{progress.projectStatus[r.project_code||'']||'Belum direncanakan'}</span>}</div></div><ArrowRight/></Link>)}</div></AppShell></AuthGate>
}
