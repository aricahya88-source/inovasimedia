...
import { CheckCircle2, Circle, Trophy, DatabaseZap } from 'lucide-react';

type Status={attempts:number;attempt_limit:number;best?:{score:number;max_score:number;percentage:number}};
...
export default function QuizPlayer({activityId}:{activityId:string}){
  ...
  const submit=async()=>{if(Object.keys(answers).length<data.questions.length&&!confirm('Masih ada soal yang belum dijawab. Tetap kirim?'))return;setBusy(true);setError('');try{const r=await api<Result>('submitStaticQuiz',{activity_id:activityId,quiz_id:data.quiz_id,answers});setResult(r);await loadStatus()}catch(e){setError(e instanceof Error?e.message:String(e))}finally{setBusy(false)}};
  ...
  return <div className="stack"><GlassCard><div className="row between wrap gap"><div><span className="eyebrow">KUIS FORMATIF</span><h2>{data.title}</h2></div><span className="badge success"><DatabaseZap/> Soal dari CDN • hasil ke Sheets</span></div><RichHtml html={data.instructions_html||''}/><div className="row wrap gap"><span className="badge">Percobaan: {status.attempts}/{status.attempt_limit}</span>{status.best&&<span className="badge success">Terbaik: {status.best.percentage.toFixed(0)}%</span>}</div></GlassCard>{data.questions.map((q,idx)=><GlassCard key={q.question_id} className="quiz-question"><div className="question-number">{idx+1}</div><RichHtml html={q.question_html}/><div className="option-list">{q.options.map(o=><button type="button" key={o.key} onClick={()=>setAnswers(v=>({...v,[q.question_id]:o.key}))} className={answers[q.question_id]===o.key?'quiz-option-button selected':'quiz-option-button'}>{answers[q.question_id]===o.key?<CheckCircle2/>:<Circle/>}<strong>{o.key}</strong><RichHtml html={o.html}/></button>)}</div></GlassCard>)}{error&&<p className="error-text">{error}</p>}<button className="button primary large" disabled={busy||remaining<=0} onClick={submit}>{busy?'Mengoreksi...':remaining<=0?'Kesempatan habis':'Kirim Jawaban'}</button></div>;
}
